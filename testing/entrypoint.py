        ...
        for filename in os.listdir(args.uploads):
            ...
            if os.path.isfile(filepath):
                ...
                if os.path.exists(transcript_output):
                    # logging.info(f"Skipping {filename}, transcript already exists.")
                    time.sleep(60)
                    continue

                # Construct the command
                command = f"insanely-fast-whisper --file-name '{filepath}' --transcript-path '{transcript_output}' --batch-size '{args.batch_size}'"
