...
def main():
    parser = argparse.ArgumentParser(
        description="Monitors a directory for file changes and processes files."
    )
    parser.add_argument(
        "-u",
        "--uploads",
    ...
    parser.add_argument(
        "-l", "--logs", default=DEFAULT_LOGS, help="Directory to save log files"
    )
    parser.add_argument(
        "-b",
        "--batch-size",
    ...
    parser.add_argument(
        "-m", "--model", default=DEFAULT_MODEL, help="Model to be used for processing"
    )
    parser.add_argument(
        "--processed-txt-dir",
        default=DEFAULT_PROCESSED_TXT_DIR,
        help="Directory to save processed txt files",
    )
    parser.add_argument(
        "--processed-srt-dir",
        default=DEFAULT_PROCESSED_SRT_DIR,
        help="Directory to save processed srt files",
    )
    parser.add_argument(
        "--convert", action="store_true", help="Convert JSON to an output format"
    )
    ...
    try:
        create_directories(
            [
                ...
                args.transcripts,
                args.logs,
                args.processed_txt_dir,
                args.processed_srt_dir,
            ]
        )
        ...
        if args.convert:
            ...
            log_file_path = os.path.join(
                args.logs,
                f"{os.path.splitext(os.path.basename(args.input_file))[0]}.log",
            )
                ...
                for filename in os.listdir(args.uploads):
                    ...
                    if os.path.isfile(filepath):
                        ...
                        transcript_output = os.path.join(
                            args.transcripts, f"{os.path.splitext(filename)[0]}.json"
                        )
                        log_file_path = os.path.join(
                            args.logs, f"{os.path.splitext(filename)[0]}.log"
                        )
                        ...
                        if result != 0:
                            ...
                            logging.info(
                                f"{filename} processed successfully. Log saved to {log_file_path}."
                            )
